...
import { Download, Save, Copy, Undo2, Upload, Calendar as CalendarIcon } from 'lucide-react';
          ...
          <table>
            ...
            <tbody>
              {employees.map((e: any, i: number) => {
                ...
                return <tr key={i}><td style={{ fontWeight: 600 }}>{e.name}<div className="mono" style={{ fontSize: 10, color: 'var(--t3)' }}>{e.shift}</div></td><td className="mono">18</td><td className="mono">1</td><td className="mono">1</td><td className="mono">0</td><td className="mono">2</td><td className="mono">22</td><td className="mono" style={{ fontWeight: 600, color: pct >= 90 ? 'var(--green)' : pct >= 75 ? 'var(--amber)' : 'var(--red)' }}>{pct}%</td></tr>;
              })}
            </tbody>
          </table>
        </div>
